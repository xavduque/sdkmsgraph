...
from app.auth_helper import get_sign_in_flow, get_token_from_code, store_user, remove_user_and_token, get_token
...
def users(request):
    ...
    usuarios = get_users_all(token)

    if usuarios:
        pass

    return render(request, 'app/calendar.html', usuarios)


def new_user(request):
    context = initialize_context(request)
    token = get_token(request)
    usuario = user_create(token)
...
def newevent(request):
    ...
    if request.method == 'POST':
        ...
        if (not request.POST['ev-subject']) or \
            ...
            return render(request, 'app/newevent.html', context)
